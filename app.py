...
def create():
    ...
    if request.method == "POST":
        ...
        con.execute("UPDATE users SET xp=xp+10 WHERE username=?", (session["user"],))
        con.commit()
        return redirect("/")
...
def post(pid):
    ...
    if request.method == "POST" and "user" in session:
        ...
        con.execute("UPDATE users SET xp=xp+2 WHERE username=?", (session["user"],))
        con.commit()

    p = con.execute("SELECT * FROM posts WHERE id=?", (pid,)).fetchone()
    comments = con.execute("SELECT * FROM comments WHERE post_id=?", (pid,)).fetchall()
    return render_template("post.html", post=p, comments=comments, user=session.get("user"))
...
def vote(pid):
    if "user" not in session:
        return redirect("/login")
    con = get_db()
    con.execute("UPDATE posts SET votes=votes+1 WHERE id=?", (pid,))
    con.commit()
    return redirect("/")
